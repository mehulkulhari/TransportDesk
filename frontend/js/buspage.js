...
export async function loadBusPage(bus){
  ...
  const rows=(roster||[]).map(r=>{const f=fuelBy[r.sr_no]||{};
    return `<tr><td>${r.pickup_order??'—'}</td><td>${esc(r.student_name)}</td><td class="mono">${esc(r.sr_no)}</td><td>${f.ride_min!=null?f.ride_min+' min':(r.road_min_to_school!=null?r.road_min_to_school+' min':'—')}</td><td>${f.ride_km!=null?f.ride_km+' km':'—'}</td><td>${f.fuel_share!=null?rs(f.fuel_share)+'/yr':'—'}</td></tr>`;}).join('');
  $('bpBody').innerHTML=`
    <div class="cards" style="margin-bottom:16px">

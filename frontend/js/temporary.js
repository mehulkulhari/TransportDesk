...
// Round 2 keeps its children in students_round2, and student_temp_assignments has a hard FK
// to students(id). Round 2 therefore writes to its own parallel table with the same checks.
const r2 = () => globalThis.tdRound === 2;
const tempTable = () => r2() ? 'student_temp_assignments_r2' : 'student_temp_assignments';

export async function searchTemp(term){term=term.trim();
  if(term.length<2){$('tresults').innerHTML='<div class="hint">Search a student.</div>';return;}
  const data=await searchRoundStudents(term,40);   // follows the selected round
  renderList($('tresults'),data,openTemp);}

export async function openTemp(id){
  let data;
  if(r2()){
    const {data:k}=await db.from('students_round2').select('*').eq('id',id).single();
    if(!k){toast('Child not found','bad');return;}
    data={...k, student_name:k.name, permanent_bus:k.bus_no,
          permanent_latitude:k.latitude, permanent_longitude:k.longitude,
          latitude:k.latitude, longitude:k.longitude};
  }else{
    ({data}=await db.from('student_effective').select('*').eq('id',id).single());
  }
  const busIds=await roundBusIds();
  const busOpts=busIds.map(b=>`<option value="${b}">Bus ${b}</option>`).join('');
  const today=new Date().toISOString().slice(0,10);
  const {data:active}=await db.from(tempTable()).select('*').eq('student_id',id).or(`valid_until.is.null,valid_until.gte.${today}`).order('valid_from',{ascending:false});
  ...
  if($('endTemp'))$('endTemp').onclick=async()=>{await db.from(tempTable()).update({valid_until:today}).eq('student_id',id).or(`valid_until.is.null,valid_until.gte.${today}`);toast('Ended','good');openTemp(id);loadDashboard();};
  ...
  $('tSave').onclick=async()=>{const bus=$('tBus').value?parseInt($('tBus').value,10):null;
    ...
    const {error}=await db.from(tempTable()).insert({student_id:id,temp_bus_no:bus,temp_latitude:tlat,temp_longitude:tlon,valid_from:$('tFrom').value||today,valid_until:$('tUntil').value||null,reason:$('tReason').value.trim()||null});
    if(error){toast(/temp_(r2_)?no_overlap/.test(error.message)?'Overlaps an existing temporary arrangement':error.message,'bad');return;}
...
Object.assign(globalThis, { searchTemp, openTemp });

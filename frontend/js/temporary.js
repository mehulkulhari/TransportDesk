...
export async function searchTemp(term){term=term.trim();
  // Temporary arrangements hang off students.id via student_temp_assignments, which has no
  // Round-2 side yet. Say so plainly rather than listing Round-1 students under Round 2.
  if(globalThis.tdRound===2){
    $('tresults').innerHTML='<div class="hint">Temporary arrangements are Round&nbsp;1 only for now.<br><br>Round&nbsp;2 children are held in their own table, which the temporary-assignment records do not cover yet. Switch to Round&nbsp;1 from the header to set one.</div>';
    return;}
...
export function initTemporary() {
  if(globalThis.tdRound===2){
    $('tq').disabled=true; $('tq').placeholder='Round 1 only';
    searchTemp('');
    return;
  }
  let timer;
